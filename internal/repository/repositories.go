...
func (r *Repository) GetAllStreams() ([]models.Stream, error) {
	rows, err := r.db.Query(`SELECT id, name, slug, tvg_id, tvg_name, tvg_chno, media_url, logo_id, logo_url,
		program_id, COALESCE(local_addr, ''), mode, idle_timeout_sec, buffer_size, fifo_size,
		COALESCE(analyze_duration, ''), COALESCE(probe_size, ''), use_gpu, overrun_nonfatal, enabled,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.Name, &s.Slug, &s.TVGId, &s.TVGName, &s.TVGChno, &s.MediaURL, &s.LogoID, &s.LogoURL,
			&s.ProgramID, &s.LocalAddr, &s.Mode, &s.IdleTimeoutSec, &s.BufferSize, &s.FifoSize,
			&s.AnalyzeDuration, &s.ProbeSize, &useGPUInt, &overrunInt, &enabledInt,
...
func (r *Repository) CreateStream(s *models.Stream) (*models.Stream, error) {
	...
	_, err := r.db.Exec(`INSERT INTO streams (id, name, slug, tvg_id, tvg_name, tvg_chno, media_url, logo_id, logo_url,
		program_id, local_addr, mode, idle_timeout_sec, buffer_size, fifo_size, analyze_duration, probe_size,
		use_gpu, overrun_nonfatal, enabled, auto_recover, recover_timeout_sec, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		s.ID, s.Name, s.Slug, s.TVGId, s.TVGName, s.TVGChno, s.MediaURL, s.LogoID, s.LogoURL,
		s.ProgramID, s.LocalAddr, s.Mode, s.IdleTimeoutSec, s.BufferSize, s.FifoSize, s.AnalyzeDuration, s.ProbeSize,
		useGPUInt, overrunInt, enabledInt, autoRecoverInt, s.RecoverTimeoutSec, s.CreatedAt, s.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) UpdateStream(s *models.Stream) error {
	...
	_, err := r.db.Exec(`UPDATE streams SET name = ?, slug = ?, tvg_id = ?, tvg_name = ?, tvg_chno = ?, media_url = ?,
		...
		analyze_duration = ?, probe_size = ?, use_gpu = ?, overrun_nonfatal = ?, enabled = ?, auto_recover = ?, recover_timeout_sec = ?, updated_at = ? WHERE id = ?`,
